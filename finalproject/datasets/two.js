...
function LoadJSONDataFile() {
    GetContentUsingAJAX("https://data.calgary.ca/resource/k7p9-kppz.json");
        ...
        if (searchInput && parseInt(searchInput)) {
            if (searchInput && searchInput.length > 0) {
                ...
                for (let idx = 0; idx < SearchableClients.length; idx++) {
                    ...
                    if (clientRecord) {
                        if (searchInput.length >= 1 && clientRecord.camera_url.description) {
            ...
            for (let idx = 0; idx < SearchableClients.length; idx++) {
                ...
                if (clientRecord) {
                    ...
                    if (searchInput.length >= 1) {
                        if (clientRecord.quadrant.match(matchNum)) {
                            // found another match
                            rowId++;
                            var match2 = OutputDataRow(clientRecord, rowId);
                            htmlFragment += match2;
        ...
        if (searchInput && searchInput.length > 0) {
            ...
            for (let idx = 0; idx < SearchableClients.length; idx++) {
                ...
                if (clientRecord) {
                    var matchNum = new RegExp(searchInput, 'gi');
                    if (searchInput.length >= 1) {
                        if (clientRecord.camera_location.match(matchNum)) {
                            // found another match
                            rowId++;
                            var match2 = OutputDataRow(clientRecord, rowId);
                            htmlFragment += match2;
...
function GridHeaderInfo(showTotalRecords) {
    ...
    <div id='hdr-clientData' class='header'>
        ...
        <span id='hdr-desc'>Camera Location</span>
        <span id='hdr-name'>Region</span>
        <span id='hdr-city'>Number</span>
        <span id='hdr-city'>Location</span>
        <span id='hdr-city'>Camera</span>
    </div>
    <br/>
            `;
    return htmlFragment;
...
function OutputDataRow(clientRecord, rowId) {
    ...
    var googleMap = "https://www.google.ca/maps/place/" + clientRecord.point.coordinates[1] + "," + clientRecord.point.coordinates[0];

    var camURL = clientRecord.camera_url.url;

    var rowHtmlFragment = `
    <div id='row-${rowId}' class='row ${evenRow}'>
    <span class='row-rowid'>${rowId}</span>
    <span class='row-desc'>${clientRecord.camera_location}</span>
    <span class='row-name'>${clientRecord.quadrant}</span>
    <span class='row-city'>
            ${clientRecord.camera_url.description}
    </span>
    <span id='hdr-city'>
    <a href="${googleMap}" target="_blank">See on Map</a>
    </span>
    <span id='hdr-city'>
    <a href="${camURL}" target="_blank">See Image</a>
    </div> <br>                  
            `;
