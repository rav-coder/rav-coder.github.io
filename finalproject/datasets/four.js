...
function LoadJSONDataFile() {
    GetContentUsingAJAX("https://data.calgary.ca/resource/c2es-76ed.json");
        ...
        if (searchInput && parseInt(searchInput)) {
            if (searchInput.length > 0) {
                ...
                for (let idx = 0; idx < SearchableClients.length; idx++) {
                    ...
                    if (clientRecord) {
                        if (searchInput.length == 4 && clientRecord.permitclass) {
                            var date = clientRecord.permitclass.substring(0, 4);
                            if ((date == searchInput)) {
                                // found another match
                                rowId++;
        ...
        if (searchInput && searchInput.length > 0) {
            ...
            for (let idx = 0; idx < SearchableClients.length; idx++) {
                ...
                if (clientRecord) {
                    ...
                    if (searchInput.length >= 1 && clientRecord.communityname) {
                        if (clientRecord.communityname.match(matchNum)) {
                            // found another match
                            rowId++;
                            var match2 = OutputDataRow(clientRecord, rowId);
                            htmlFragment += match2;
            ...
            for (let idx = 0; idx < SearchableClients.length; idx++) {
                ...
                if (clientRecord) {
                    ...
                    if (searchInput.length >= 1) {
                        if (clientRecord.workclass.match(matchNum)) {
                            // found another match
                            rowId++;
                            var match2 = OutputDataRow(clientRecord, rowId);
                            htmlFragment += match2;
...
function GridHeaderInfo(showTotalRecords) {
    ...
    <div id='hdr-clientData' class='header'>
        ...
        <span id='hdr-desc'>Permit Type</span>
        <span id='hdr-desc'>Permit Class</span>
        <span id='hdr-city'>Work Class</span>
        <span id='hdr-address'>Community</span>
...
function OutputDataRow(clientRecord, rowId) {
    ...
    if (clientRecord.communityname == undefined) {
        clientRecord.communityname = "-- No Data --";
    }

    var rowHtmlFragment = `
    <div id='row-${rowId}' class='row ${evenRow}'>
    <span class='row-rowid'>${rowId}</span>
    <span class='row-desc'>${clientRecord.permittype}</span>
    <span class='row-desc'>${clientRecord.permitclass}</span>
    <span class='row-city'>
            ${clientRecord.workclass}
    </span>
    <span class='row-address'>
            ${clientRecord.communityname}
    </span>
    <span id='hdr-city'>
