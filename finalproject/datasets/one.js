...
function LoadJSONDataFile() {
    GetContentUsingAJAX("https://data.calgary.ca/resource/35ra-9556.json");
    ...
    if (element && element.value !== undefined) {
        ...
        if (searchInput && parseInt(searchInput)) {
            if (searchInput && searchInput.length == 1) {
                document.getElementById('SearchResults').innerHTML = "<span> Please enter the number in two-digit format.</span>";
            } else
            if (searchInput && searchInput.length > 0) {
                ...
                for (let idx = 0; idx < SearchableClients.length; idx++) {
                    ...
                    if (clientRecord) {
                        ...
                        if (searchInput.length >= 2 && clientRecord.start_dt) {
                            var date = clientRecord.start_dt.substring(8, 10);
                            if ((date).match(matchNum)) {
                                // found another match
                                rowId++;
            ...
            document.getElementById('SearchResults').innerHTML = "<span> No matches found. Please enter a number. </span>";
        }
...
/* function SearchForID() {
    ...
    if (element && element.value !== undefined) {
        ...
        if (searchInput && searchInput.length == 1) {
            htmlFragment += "<span> Please enter the day in two-digit format. </span>";
        ...
        if (searchInput && searchInput.length > 0) {
            ...
            for (let idx = 0; idx < SearchableClients.length; idx++) {
                ...
                if (clientRecord) {
                    var matchNum = new RegExp(searchInput, 'gi');
                    if (searchInput.length >= 1) {
                        if (clientRecord.description.match(matchNum)) {
                            // found another match
                            rowId++;
                            var match2 = OutputDataRow(clientRecord, rowId);
                            htmlFragment += match2;
        ...
        if (searchInput && searchInput.length > 0) {
            ...
            for (let idx = 0; idx < SearchableClients.length; idx++) {
                ...
                if (clientRecord) {
                    var matchNum = new RegExp(searchInput, 'gi');
                    if (searchInput.length >= 1) {
                        if (clientRecord.incident_info.match(matchNum)) {
                            // found another match
                            rowId++;
                            var match2 = OutputDataRow(clientRecord, rowId);
                            htmlFragment += match2;
...
function GridHeaderInfo(showTotalRecords) {
    ...
    <div id='hdr-clientData' class='header'>
        ...
        <span id='hdr-desc'>Description</span>
        <span id='hdr-name'>Date of Incident</span>
        <span id='hdr-info'>Incident Info</span>
...
function OutputDataRow(clientRecord, rowId) {
    ...
    var googleMap = "https://www.google.ca/maps/place/" + clientRecord.latitude + "," + clientRecord.longitude;
    ...
    <span class='row-desc'>${clientRecord.description}</span>
    <span class='row-name'>${clientRecord.start_dt.substring(0,10)}</span>
    <span class='row-info'>
        <address>
            ${clientRecord.incident_info}
        </address>
    </span>
    <span id='hdr-city'>
